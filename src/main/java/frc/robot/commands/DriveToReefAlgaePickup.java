...
public class DriveToReefAlgaePickup extends Command {
    ...
    public DriveToReefAlgaePickup(SwerveDrivetrain drive) {
        this.drive = drive;
    ...
    public void initialize() {
        ...
        if (alliance == Alliance.Blue) {
            for (Pose2d pose : DrivetrainConstants.blueAlgaePickupPosition) {
                reefPoses.add(pose);
            }
        } else {
            for (Pose2d pose : DrivetrainConstants.redAlgaePickupPosition) {
                reefPoses.add(pose);
            }
        }
